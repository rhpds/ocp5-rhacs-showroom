...
[############################] 8/8  Lab environment setup complete
...
NOTE: Vulnerable demo apps are synced by the `roadshow-demo-apps` Argo CD Application. PodSecurity warnings on those workloads are expected.
...
- *Network Graph:* Network flows, listening endpoints, and policy simulation — xref:acs-06-network-and-runtime.adoc[06 Network and Runtime Security]
- *Violations:* Policy violation events and enforcement status — xref:acs-06-network-and-runtime.adoc[acs-06]
- *Compliance:* CIS, PCI-DSS, and STIG coverage — xref:acs-04-compliance.adoc[04 Compliance]
- *Vulnerability Management:* Workload, platform, and node CVE views — xref:acs-01-vulnerability-management.adoc[01 Vulnerability Management]
- *Configuration Management:* Misconfiguration and standards reporting — xref:acs-02-policy-and-risk.adoc[02 Policy and Risk]
- *Risk:* Prioritized workloads — xref:acs-02-policy-and-risk.adoc[02 Policy and Risk]
...
Connect image registries, notifiers, backup targets, cloud sources, and API tokens. You will configure notifiers and API tokens in xref:acs-05-notifications-api.adoc[05 Notifications, API, and Integrations].
...
Setup is done and you can navigate the RHACS console. Next: xref:acs-01-vulnerability-management.adoc[Vulnerability Management].
