...
* RHACS Central reachable (xref:acs-00-setup-install-navigation.adoc[acs-00])
...
TIP: Same click path as xref:acs-06-network-and-runtime.adoc#runtime-enforce[acs-06 runtime enforce], different policy name.
====
...
RHACS runtime enforce **blocks the violating action** by killing the pod. That is EoP/Tampering at runtime, not a new detection product. Build-time is removing the packager from the image (xref:101-11-rebuilding-images.adoc[101-11], xref:201-04-shift-left-pipeline.adoc[201-04]). Deploy-time is xref:acs-02-policy-and-risk.adoc[acs-02].
