...
The scored path is the seed script. That script also adds `renovate-bot`, which xref:tssc-07-live-renovate.adoc[3.3] requires. Red Hat Developer Hub (RHDH) `lightwell-java-service` is an optional golden path, not this Check.

Do not change `Dockerfile` `FROM` here. That is xref:tssc-06-bind-base-deps.adoc[3.2]. Do not merge a fake Renovate PR. That is xref:tssc-07-live-renovate.adoc[3.3].
...
image::gitea-templates-vs-learner-org.png[workshop-templates versus your lw-student remotes,width=700,link=self,window=blank]
...
* link:{docs-rhdh-url}[Getting started with Red Hat Developer Hub^] (golden path / Software Templates)
