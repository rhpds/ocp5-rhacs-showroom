...
Runtime process enforcement (for example, blocking `apk` in a running pod) is xref:acs-06-network-and-runtime.adoc[06 Network and Runtime Security]. Build-time gates with `roxctl image check` are xref:acs-03-cicd-and-automation.adoc[03 CI/CD].
...
Have fun with xref:acs-03-cicd-and-automation.adoc[CI/CD]!
