...
Konflux “build” on hosted Konflux is often Buildah. This cluster maps that control to Binary BuildConfig started from Tekton (`oc start-build --from-dir`). Do not add a Buildah task. The term map is in xref:appendix-konflux-mapping.adoc[Konflux mapping].
